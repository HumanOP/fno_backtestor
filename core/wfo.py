...
try:
    from quantstats._plotting import core as plot
except ImportError:
    plot = None
    print("Failed to import plotting module")
...
class Backtest(BaseBacktest):
    ...
    def optimize(self, *, start_date=None, end_date=None, **kwargs):
        ...
        print(f"Custom Backtest.optimize called with start_date={start_date}, end_date={end_date}")
        print(f"Optimization parameters: {kwargs}")
        ...
        try:
            ...
            print(f"Optimization completed successfully. Result type: {type(result)}")
            return result
        except Exception as e:
            print(f"Error in parent optimize method: {e}")
            import traceback
            traceback.print_exc()
            raise
...
class WalkForwardOptimizer:
    ...
    def optimize(self, strategy_class, timeframe, stock, optimization_params, db_path, training_candles, testing_candles, method='grid',
        ...
        if results_list and test_summary_stats:
            ...
            # if last_strategy:
            #     paths = self.get_class_source_paths(last_strategy)
            #     for name, path in paths.items():
            #         temp._store_data(name, path, self.caller_filename, code_id)
            # if '_strategy' in result_temp:
