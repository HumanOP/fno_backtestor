...
class DoubleCalendarStrategy(Strategy):
    ...
    def init(self):
        super().init()
        self.legs = {
            'leg1': {'type': 'CE', 'expiry_type': 'weekly', 'expiry_range': [2, 3], 'target_strike': 'ATM', 'action': 'SELL', 'stop_loss': None, 'take_profit': None},
            'leg2': {'type': 'PE', 'expiry_type': 'weekly', 'expiry_range': [2, 3], 'target_strike': 'ATM', 'action': 'SELL', 'stop_loss': None, 'take_profit': None},
            'leg3': {'type': 'CE', 'expiry_type': 'weekly', 'expiry_range': [7, 8], 'target_strike': 'ATM', 'action': 'BUY', 'stop_loss': None, 'take_profit': None},
            'leg4': {'type': 'PE', 'expiry_type': 'weekly', 'expiry_range': [7, 8], 'target_strike': 'ATM', 'action': 'BUY', 'stop_loss': None, 'take_profit': None}
        }
    ...
    def next(self):
        ...
        if not active_trades and pd.Timestamp(self.time).time() < pd.Timestamp("15:15:00").time():
            ...
            if not exit_reason and self.entry_spot and self.entry_premium:
                ...
                # Step 2: Roll OTM side if premium ratio < 0.65 or spot move >= 150
                    ...
                    for leg_id in otm_legs:
                        ...
                        if sell_premium < 0.65 * buy_premium:
                            ...
                            if new_sell_premium < 0.65 * buy_premium:
                                print(f"[{self.time}] Rolled premium for {leg['contract']} still below 0.65 of buy: {new_sell_premium}/{buy_premium}")
                                continue
