...
# Store data globally
option_data = []
underlying_data = []  # New: Store underlying asset data
...
class OptionChainListener:
    def __init__(
        ...
        *,
        ...
        self.underlying_data = underlying_data  # New: Reference underlying data
        self.tickers = []
        self.underlying_ticker = None  # New: Store underlying ticker
        self.pending_rows: List[Dict] = []
        self.pending_underlying_rows: List[Dict] = []  # New: Batch underlying rows
        self.last_flush = None
        self.last_underlying_flush = None  # New: Track underlying flush time
        self.running = False
            ...
            with Sender.from_conf(self.questdb_conf) as sender:
                for row in rows:
                    ...
                    if symbol not in created_tables:
                        ...
                        logger.error(f"upload for {symbol}: table  created")
    ...
    def on_underlying_tick(self, ticker):
        ...
        asyncio.create_task(self._upload_to_qdb(self.pending_underlying_rows, is_underlying=True))
    ...
    def stop(self):
        ...
        if self.profiler.get_stats():
            self.profiler.print_stats(output_unit=1e-6)
