...
# Store data globally
option_data = []
created_tables: Set[str] = set()  # Thread-safe set for tracking created tables
table_lock = threading.Lock()  # Lock for thread-safe table creation
upload_time={}
class OptionChainListener:
    def __init__(
        ...
        *,
        ...
        self.running = False
        self.executor = ThreadPoolExecutor(max_workers=4)  # Thread pool for uploads
        self.print_thread = None  # Thread for printing data
    ...
    def create_table_for_symbol(self, symbol: str):
        """Create a QuestDB table for a specific symbol in a thread."""
    ...
    def upload_to_qdb_sync(self, rows: List[Dict]):
        ...
        try:
            with Sender.from_conf(self.questdb_conf) as sender:
                for row in rows:
                    symbol = row["Symbol"].replace(".", "_")  # Sanitize symbol for table name

                    if symbol not in created_tables:
                        self.create_table_for_symbol(symbol)
                        logger.info(f" upload for {symbol}: table  created")
                ...
                sender.flush()
                logger.info(f"Uploaded {len(rows)} rows to QuestDB")
    ...
    async def _upload_to_qdb(self, rows: List[Dict]):
        """Upload a batch of rows to QuestDB in a thread."""
        if not rows:
            return
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(self.executor, self.upload_to_qdb_sync, rows)
